...
const postedDate = (date)=>new Intl.DateTimeFormat("en-GB", {
...
const getAllUploads = async()=>{
  try{
    ...
    if(ethereum){
      ...
      const uploadPortalContract = new ethers.Contract(contractAddress,contractABI,signer)
      const uploads = await uploadPortalContract.getAllUploads()
      const totalCount = await uploadPortalContract.getTotalUploads()
      const contractBalance = await provider.getBalance(uploadPortalContract.address)
      ...
      uploadPortalContract.on('NewUpload',(from,timestamp,message)=>{
        ...
        setTotalUploads(prevState=>[...prevState,{
          address: from,
          message: message,
...
const connectWallet = async() => {
  ...
  const upload = async(message)=>{
    try{
      ...
      if(ethereum){
        ...
        const signer = provider.getSigner()
        const uploadPortalContract = new ethers.Contract(contractAddress,contractABI,signer)
        state.contractBalance = await provider.getBalance(uploadPortalContract.address)
         
        const uploadTxn = await uploadPortalContract.upload(message,{gasLimit:300000})
        console.log('mining---',uploadTxn.hash)
        await uploadTxn.wait()
        console.log('mined!', uploadTxn.hash)
        console.log(uploadTxn)
        state.contractBalance = await provider.getBalance(uploadPortalContract.address)
        console.log('the contract balance is now...',ethers.utils.formatEther(state.contractBalance))
  ...
  return (
    <div className="mainContainer">
      ...
      <div className='btnHeader'>  {!state.currentAccount && (<button className="connectButton" onClick={connectWallet}>
         connect Wallet
        </button>)}{state.currentAccount && (<p>Hello, {state.currentAccount.substring(0,8)}...</p>)}</div>
        ...
        <div className="bio">
        The name is Dante, I'm the cofounder of Underscore.ave and guess what! I'm bringing the creatives and their fans to the blockchain 😎.
        you can find the link to my company's website ➡️ 
         <a href='https://www.underscoreave.com'>here</a>
        </div>
        {state.currentAccount && (<div className='contentContainer'>
        <textarea className ='comment'placeholder='Why are you interested in web3?' value={state.message} onChange={handleChange} maxLength='120'  rows="5" cols="33"/>    
       ...
       <p> There are {totalUploads.length} total comments!</p>
          ...
          return (
            <div key={index} style={{ backgroundColor: "OldLace", marginTop: "16px", padding: "8px", borderRadius:'10px'}}>
              <div className='commentHeader'>Address: {upload.address.substring(0,8)}...
